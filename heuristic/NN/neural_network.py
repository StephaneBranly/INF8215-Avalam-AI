...
class NN:
    ...
    def load_best_indiv(self,filename):
        with open(filename) as fp:
            listObj = json.load(fp)
        best = 0
        for i in range(len(listObj["gen"])): 
            if listObj["gen"][i]["score"] < listObj["gen"][best]["score"]:
                best = i
        self.name = listObj["gen"][best]["name"]
        self.weights = [np.array(a) for a in listObj["gen"][best]["weights"]]
        self.biases = [np.array(a) for a in listObj["gen"][best]["biases"]]
        self.layers = listObj["gen"][0]["layers"]